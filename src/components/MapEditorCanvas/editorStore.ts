...
const runLargePanelGreedy = (
  ...
  // 固定采用从左上到右下的扫描顺序，保证同一输入总能得到稳定布局。
...
const buildPlatformPanelLayout = (base: CellValue[], gridWidth: number, gridHeight: number) => {
  ...
  // 钢平台轮廓对“先横后竖”或“先竖后横”的结果会很敏感，所以这里全都试一遍。
  PANEL_STRATEGIES.forEach((strategy) => {
    const largePlan = runLargePanelGreedy(occupied, gridWidth, gridHeight, strategy);
...
const arePanelsEqual = (left: PlatformPanel[], right: PlatformPanel[]) =>
...
const createEditorStoreCore = () => {
  ...
  const clearPlatformPanelsInternal = () => {
    // 只要平台轮廓变了，旧的实际面板布局就不再可信，需要整体作废。
    if (project.value.overlays.platformPanels.length === 0) {
      return false;
    }
    project.value.overlays.platformPanels = [];
    return true;
  };
  ...
  const setCell = (x: number, y: number, value: CellValue) => {
    ...
    const previous = project.value.layers.base[idx];
    if (previous === value) {
      return false;
    }
    rememberSnapshot();
    ...
    if ((previous === 0) !== (value === 0)) {
      // 只有“空白 <-> 平台”切换时，才会真正影响铺板结果。
      clearPlatformPanelsInternal();
    }
  ...
  const planPlatformPanels = () => {
    const nextPanels = buildPlatformPanelLayout(project.value.layers.base, width.value, height.value);
    // 结果没变时直接返回，避免产生无意义的更新时间和撤销记录。
    if (arePanelsEqual(nextPanels, project.value.overlays.platformPanels)) {
      return false;
    }
    rememberSnapshot();
    project.value.overlays.platformPanels = nextPanels;
    markChanged();
    return true;
  };
  ...
  const deleteSelectedElement = () => {
    ...
    if (selectedElement.value.kind === "cell") {
      ...
      const changed = setCell(selectedElement.value.x, selectedElement.value.y, 0);
      if (changed) {
        // 选框工具从键盘删除平台格时，需要替换 project 引用触发底图重绘。
        forceProjectReplace();
      }
      return changed;
    }
    ...
    if (selectedElement.value.kind === "mixed-batch") {
      ...
      if (changed) {
        markChanged();
        if (cellChanged) {
          // base 数组是原位修改，这里强制替换引用，确保底图层能重绘。
          forceProjectReplace();
        }
      }
      return changed;
